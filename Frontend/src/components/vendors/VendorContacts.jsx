...
function makeId() {
  return `vendor_contact_${Date.now()}_${Math.random().toString(16).slice(2)}`;
}
...
export function VendorContacts({ contacts: propContacts }) {
  ...
  return (
    <div className="flex flex-col h-full min-w-0 gap-4">
      ...
      {deleteTarget && (
        <DeleteVendorContactConfirmModal
...
function DeleteVendorContactConfirmModal({ contact, onCancel, onConfirm }) {
  return (
