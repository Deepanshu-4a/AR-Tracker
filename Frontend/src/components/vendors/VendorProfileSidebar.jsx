...
export function VendorProfileSidebar({ vendor, onBack, onEdit }) {
  ...
  const complianceStatus = vendor.complianceStatus ?? "Compliant";
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-7 shadow-sm space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm space-y-4">
            ...
            <Metric
              ...
              value={vendor.primaryBusinessUnit || "-"}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm space-y-3">
            <Metric
              label="Total Spend"
              value={`$${Number(totalSpend).toLocaleString()}`}
            />
            <Metric
              label="YTD Spend"
              value={`$${Number(vendor.ytdSpend ?? 98000).toLocaleString()}`}
            />
            <Metric label="Open Bills" value={vendor.openBills ?? 4} />
            <Metric
              label="Payment Terms"
              value={vendor.netTerms ?? vendor.paymentTerms ?? "Net 30"}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge
                className={`px-3 py-1 rounded-full text-xs ${statusUI.tone}`}
              >
                {statusUI.label}
              </Badge>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Metric
                label="Last Review"
                value={vendor.lastComplianceReview ?? "Jan 2026"}
              />
            </div>
          </div>
        </div>
